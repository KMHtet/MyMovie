...
export default class ServicesHelperUtils {
    ...
    async initRequest(method: string, headers: any, url: string, params: any, enable_access_token: boolean) {
        const result_ =  await this.callAPI(method, headers, url, params, false);
        return result_;
    }

    async callAPI(method: string, headers: any, url: string, params: any, enable_access_token: boolean) {
